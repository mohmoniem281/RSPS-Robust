...
class EquityCurveBuilder:
    ...
    def build_equity_curve(self):
        ...
        for i in range(start_idx + 1, end_idx + 1):
            ...
            print(f"  Trade: {winner_asset} | Entry: ${entry_price:.2f} | Exit: ${exit_price:.2f} | PnL: ${pnl:.2f} | Capital: ${self.current_capital:.2f}")
...
def apply_trend_filters(config: Dict):
    ...
    for i in range(1, len(equity_points)):
        ...
        historical_capitals = [p["capital"] for p in equity_points[:i]]
        
        # Apply DEMA trend filter
        is_trending, dema_value = apply_dema_trend_filter(historical_capitals, dema_config)
        
        if not is_trending:
            # Not trending - set to cash position
            new_point = {
            ...
            # Trending - use original trade data
            ...
            if original_position is not None and original_entry_price is not None and original_exit_price is not None:
                ...
                # No valid trade data - set to cash
                new_point = {
                    "identifier": point["identifier"],
                    "capital": current_capital,
                    "pnl": 0.0,
