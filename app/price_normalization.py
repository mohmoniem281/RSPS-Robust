...
def normalize_prices(config):
    ...
    for asset_name, asset_config in config["assets"].items():
        ...
        if config["use_ema_filtering_price_normalization"]:
            #using the ema method to create normalized prices
            ...
            # Find the historical price (normalization_window periods ago)
                # Get all prices up to current identifier into a list
                prices_list = []
                for entry in price_history:
                    prices_list.append(entry["close"])
                    if entry["time"] == identifier:
                        break
                
                # Calculate EMA
                ema = calculate_ema(prices_list).iloc[-1]
                

                # Calculate normalized price
                normalized_price = current_price / ema -1
                
                normalized_prices.append({
                    "time": identifier,
                    "normalized_price": normalized_price,
                    "current_price": current_price,
                    "historical_price": "using ema method. NA",
                    "historical_identifier": "using ema method. NA"
                })
        else:
